...
public class AccountSearchRequestDto {
    ...
    private Boolean isDormant; //체크박스 - 휴면계좌여부(YES, NO, ALL)
    private AccountWhether isDummy;   //체크박스 - 더미여부(YES, NO, ALL)

    private AccountKeywordType keywordType; // 검색키워드타입 (계좌이름별, 계좌번호별, 고객이름별, 상품이름별, 더미이름별)
    private String searchKeyword; //검색명

    private AccountSortBy sortBy = LATEST;   //정렬 (잔액적은순/많은순, 최신순/과거순, 만기일순)
}